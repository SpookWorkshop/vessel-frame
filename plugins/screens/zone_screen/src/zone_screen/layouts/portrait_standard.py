...
class PortraitStandard(ZoneLayout):
    """Single-column portrait layout (compact + standard tiers)."""

    async def render(self) -> None:
        canvas = self._renderer.canvas
        self._setup_responsive(*canvas.size)
    ...
    def _setup_responsive(self, w: int, h: int) -> None:
        ...
        for _ in range(FIT_MAX_STEPS):
            self._apply_scale(s)
            self._build_fonts(s)
            if self.min_height() <= h or s <= MIN_SCALE:
                break
            s *= SCALE_SHRINK
